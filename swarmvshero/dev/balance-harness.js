...
//
// Last measured (480 runs pooled for DUMB): DUMB 53%, STAGE 28%, SMART 20%.
// Parking the army out of the fight loses; constant pressure is the baseline.
// Individual n=80 samples of an identical config ranged 38-64%, so never tune
// on one sample — pool several hundred runs before moving a number.
//
...
window.__check = (g) => {
  ...
  if (g.heroBoons.length > 6) bad.push(`boons ${g.heroBoons.length}`);
  for (const b of g.heroBoons) {
    if (!Number.isFinite(b.timer)) bad.push('boon NaN timer');
    if (b.timer > b.def.duration + 0.001) bad.push(`boon over duration ${b.def.id}`);
  }
  if (g.allies.length > 8) bad.push(`allies ${g.allies.length}`);
  for (const a of g.allies) {
    if (!Number.isFinite(a.x) || !Number.isFinite(a.y) || !Number.isFinite(a.hp)) { bad.push('ally NaN'); break; }
    if (a.hp > a.maxHp + 1) { bad.push('ally hp>max'); break; }
    if (a.x < 0 || a.y < 0 || a.x > g.world.width || a.y > g.world.height) { bad.push('ally out of world'); break; }
  }
  if (g.roster.some((id, i) => id !== null && !window.__SLOTS[i].includes(id))) bad.push('roster corrupted');
...
window.__suite = (bot, n, opts) => {
  ...
  return {
    ...
    byStrain: (() => {
      const m = {};
      for (const r of out) {
        m[r.strains] = m[r.strains] ?? { w: 0, n: 0 };
        m[r.strains].n += 1;
        if (r.res === 'victory') m[r.strains].w += 1;
      }
      return Object.fromEntries(Object.entries(m).map(([k,v]) => [k, `${v.w}/${v.n}`]));
    })(),
